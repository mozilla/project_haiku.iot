/*********************************************************************
 Modified from example nRF51822 based Bluefruit LE modules
...
void setup(void)
{
  ...
  /* Send AT commands to check power level and change it to 4dbm
     By default powerlevel of BLE SPI Shiedl is set to 0 
     and AT+BLEGETRSSI return 0 if not connected to external device*/
  ble.println("AT+BLEPOWERLEVEL");
  ble.waitForOK();
  ble.println("AT+BLEGETRSSI");
  ble.waitForOK();
  ble.println("AT+BLEPOWERLEVEL=0");
  ble.waitForOK();
  delay(2000);
